...
class JumpstartSitesEngineering extends JumpstartSitesAcademic {
  ...
  public function get_install_tasks(&$install_state) {
    ...
    $tasks['jse_configure_person_cap_display'] = array(
      'display_name' => st('Configure CAP dispay for people pages'),
      'display' => FALSE,
      'type' => 'normal',
  ...
  public function install(&$install_state) {
    $time = time();
    drush_log('JSE - Starting Install', 'ok');
    ...
    drush_log('JSE - Finished installing redirects.', 'ok');
    $time_diff = time() - $time;
    drush_log('JSE - Finished install task. Time: ' . $time_diff . ' seconds', 'ok');
  }
   ...
   * @return [type]                [description]
   */
  public function configure_person_cap_display(&$install_state) {
    $time = time();
    drush_log('JSE - Configuring person CAP displays.' . $time, 'ok');
